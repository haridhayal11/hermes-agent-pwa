import { POST as select } from "@/app/api/projects/[id]/sessions/[sessionId]/select/route";
...
export async function POST(request: Request, ctx: Context) {
  return withDevice(request, async (device) => {
    ...
    return idempotentJson(
      request,
      device.id,
      `POST:/api/v1/projects/${id}/sessions/${sessionId}/select`,
      async () => fromLegacy(await select(request, ctx)),
    );
  });
}
